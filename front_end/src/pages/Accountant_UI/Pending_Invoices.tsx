...
import { faArrowUpWideShort, faArrowDownShortWide, faL } from '@fortawesome/free-solid-svg-icons';
...
const Pending_Accountant_Invoices: React.FC = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>([
    'customer_name', 'ref_no', 'date', 'pending', 'due_on', 'days_passed','sales_person'
  ]);
  ...
  const get_invoices = async () => {
    setLoading(true)
    try {
      const response = await fetch('http://159.89.160.186/api/accountant_pending_invoice_data/', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className='font-sans flex h-screen w-screen justify-between items-center'>
      ...
      <Accountant_Side_Bar current_page='Pending_Invoices' />
        ...
        {show_invoice_popup ? (
          ...
          <div className='h-full overflow-y-auto no-scrollbar w-full flex flex-col py-6 space-y-6'>
            ...
            <div className='flex w-full h-auto items-center justify-between px-10'>
              <div className='text-4xl font-bold'>Pending Invoices</div>
            ...
            <table className='w-full overflow-x-auto border border-gray-300'>
              <thead>
                <tr>
                  ...
                  {visibleColumns.includes('due_on') && (
                    <th className='px-4 py-3 text-left text-sm border-r border-gray-300 font-normal text-gray-400'>
                      DUE DATE
                    </th>
                  )}
...
export default Pending_Accountant_Invoices;
