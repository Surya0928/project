...
const Accountant_Invoices: React.FC = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>([
    'customer_name', 'ref_no', 'date', 'pending', 'days_passed', 'paid', 'paid_date','sales_person'
  ]);
  ...
  const get_invoices = async () => {
    setLoading(true)
    try {
      const response = await fetch('http://159.89.160.186:8000/accountant_invoice_data/', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className='font-sans flex h-screen w-screen justify-between items-center'>
      ...
      <Accountant_Side_Bar current_page='All_Invoices' />
...
export default Accountant_Invoices;
