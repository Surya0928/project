...
import InvoiceInfo from '../../components/Manager_Components/Manager_Invoice_Info';
...
const Manager_Invoices: React.FC = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>([
    'customer_name', 'ref_no', 'date', 'pending', 'days_passed', 'paid', 'paid_date','sales_person','ar_accountant'
  ]);
  ...
  const get_invoices = async () => {
    setLoading(true)
    try {
      const response = await fetch('http://159.89.160.186:8000/manager_invoice_data/', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className='font-sans flex h-screen w-screen justify-between items-center'>
      ...
      <Manager_Side_Bar current_page='Invoices' />
      <div className='flex flex-col w-full h-screen'>
        <ManagerHeadBar />
        {show_invoice_popup ? (
          <div className='w-full h-3/4'>{show_invoice_popup && selectedInvoice && (<InvoiceInfo customer_name={selectedInvoice.customer_name} invoice_name={selectedInvoice.ref_no} ar_accountant={selectedInvoice.ar_accountant} accountants={accountants} sales_data={sales_data} onClose={() => {set_show_invoice_popup(false); setSelectedInvoice(null)}}/>)}</div>
        ) : (
          <div className='h-full overflow-y-auto no-scrollbar w-full flex flex-col py-6 space-y-6'>
            ...
            <div className='flex w-full h-auto items-center justify-between px-10'>
              <div className='text-4xl font-bold'>Invoices</div>
            ...
            <table className='w-full overflow-x-auto border border-gray-300'>
              <thead>
                <tr>
                  ...
                  {visibleColumns.includes('days_passed') && (
                    <th className='px-4 py-3 text-left text-sm border-r border-gray-300 font-normal text-gray-400'>
                      DAYS PASSED
                    </th>
                  )}
              ...
              <tbody>
                {filteredInvoices.map((invoice, index) => (
                  ...
                  >
                    ...
                    {visibleColumns.includes('ar_accountant') && (
                      <td className='px-4 py-3 text-sm text-gray-500 border-r border-gray-300'>
                        {accountants.find((acc) => acc.id === invoice.ar_accountant)?.username || '-'}
                      </td>
                    )}
                  </tr>
                ))}
...
export default Manager_Invoices;
