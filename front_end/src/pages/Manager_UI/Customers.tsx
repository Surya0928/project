...
import CustomerInfo from '../../components/Manager_Components/Manager_Customer_Info';
...
const Manager_Customers: React.FC = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>([
    'customer_name', 'invoices', 'credit_period', 'over_due', 'total_balance', 'ar_accountant'
  ]);
  ...
  const get_customers = async () => {
    setLoading(true)
    try {
      const response = await fetch('http://159.89.160.186:8000/manager_customer_data/', {
        method: 'POST',
        headers: {
  ...
  const handleFilterChange = (field: string, operator: string, value: string) => {
  ...
  const isSubsequence = (sub: string, str: string): boolean => {
    ...
    for (const char of lowerStr) {
      if (char === lowerSub[subIndex]) {
        subIndex++;
        if (subIndex === lowerSub.length) {
          return true;
        }
      }
    }
  ...
  const filteredCustomers = sortedCustomers
    ...
    .filter((customer) => selectedAccountant === null || customer.ar_accountant === selectedAccountant)
  ...
  return (
    <div className='font-sans flex h-screen w-screen justify-between items-center'>
      ...
      <Manager_Side_Bar current_page='Customers' />
      <div className='flex flex-col w-full h-screen'>
        <ManagerHeadBar />
        {show_customer_popup ? (
          <div className='w-full h-full overflow-y-auto py-2'>{show_customer_popup && selectedCustomer && (<CustomerInfo customer_name={selectedCustomer.customer_name} ar_accountant={selectedCustomer.ar_accountant} accountants={accountants} sales_data={sales_data} onClose={() => {set_show_customer_popup(false); setSelectedCustomer(null)}}/>)}</div>
        ) : (
            ...
            <table className='w-full overflow-x-auto border border-gray-300'>
              <thead>
                <tr>
                  ...
                  {visibleColumns.includes('ar_accountant') && (
                    <th className='px-4 py-3 text-left text-sm border-r border-gray-300 font-normal text-gray-400'>
                      A/R ACCOUNTANT
                    </th>
                  )}
              ...
              <tbody>
                {filteredCustomers.map((customer, index) => (
                  ...
                  >
                    ...
                    {visibleColumns.includes('ar_accountant') && (
                      <td className='px-4 py-3 text-sm text-gray-500 border-r border-gray-300'>
                        {accountants.find((acc) => acc.id === customer.ar_accountant)?.username || 'N/A'}
                      </td>
                    )}
                  </tr>
                ))}
