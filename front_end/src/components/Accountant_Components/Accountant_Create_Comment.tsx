...
const AccountantCreateComment: React.FC<CustomerInfoProps> = ({ customer_name,invoices,  sales_data, onClose }) => {
  ...
  const fetchCustomerData = async () => {
    set_loading(true)
    try {
      const response = await fetch('http://159.89.160.186:8000/get_create_comment/', {
        method: 'POST',
        headers: {
  ...
  const create_commentt = async () => {
    ...
    if (selected_invoices.length >0 && selectedOption && (promised_date || follow_up_date || paid_date)) {
      try {
...
export default AccountantCreateComment;
