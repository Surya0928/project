...
interface AccountantCustomerInfoProps {
  customer_name: string;
...
const AccountantCustomerInfo: React.FC<AccountantCustomerInfoProps> = ({ customer_name, sales_data, onClose }) => {
  const {id} = useAppContext()
  const [selected_customer, set_selected_customer] = useState<Customer>({
    id: 0,customer_name: '',invoices: 0,credit_period: 0,over_due: 0,total_balance: 0
    })
  ...
  const handleSaveCreditPeriod = async () => {
    try {
      const response = await fetch('http://159.89.160.186:8000/customer_credit_period/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleAddName = async () => {
    try {
      const response = await fetch('http://159.89.160.186:8000/customer_name_number/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div className='relative flex w-full h-full'>
      {show_add_comment_popup || show_invoice_popup ? (
        <div className='w-full h-full'>
          {show_add_comment_popup && invoice_data && (<AccountantCreateComment customer_name={customer_name} invoices={invoice_data} sales_data={sales_data} onClose={()=>(set_show_add_comment_popup(false), fetchCustomerData())} />)}
          {show_invoice_popup && (
            <div className='w-full h-full'>{show_invoice_popup && (<AccountantInvoiceInfo customer_name={customer_name} invoice_name={selected_invoice} sales_data={sales_data} onClose={() => {set_show_invoice_popup(false)}}/>)}</div>
          )}
          </div>
                            ...
                            <tbody>
                              {comment_data?.slice(0, 2).map((comment) => (
                                <tr key={comment.id} className="border-t">
                                    ...
                                    <td className="p-2 border-r border-t border-gray-200">
                                      {comment.follow_up_date
                                          ? `${comment.follow_up_date}${comment.follow_up_time ? `, ${comment.follow_up_time}` : ''}`
                                      : '-'}
                                    </td>
                          ...
                          <table className="w-full italic text-center">
                              ...
                              {invoice_data?.filter(invoice => {
                                  ...
                                  <tr key={invoice.id} className="border-t">
                                      ...
                                      <td className="px-4 py-2 hover:text-blue-400 border-r border-gray-200 cursor-pointer" onClick={()=>{set_selected_invoice(invoice.ref_no); set_show_invoice_popup(true)}}>{invoice.ref_no}</td>
...
export default AccountantCustomerInfo;
