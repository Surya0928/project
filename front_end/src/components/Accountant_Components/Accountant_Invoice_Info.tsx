...
interface AccountantInvoiceInfoProps {
  customer_name: string;
  invoice_name: string;
...
const AccountantInvoiceInfo: React.FC<AccountantInvoiceInfoProps> = ({ customer_name, invoice_name, sales_data, onClose }) => {
  ...
  const fetchInvoiceData = async () => {
    setloading(true)
    try {
      const response = await fetch('http://159.89.160.186/api/get_invoice/', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    fetchInvoiceData();
  }, [selected_invoice.customer_name]);
    ...
    <div className='relative flex w-full h-full'>
      {show_customer_popup || show_add_comment_popup ? (
        <div className='w-full h-full'>
          {show_customer_popup && (
            <div className='w-full h-full'>{show_customer_popup && (<AccountantCustomerInfo customer_name={customer_name} sales_data={sales_data} onClose={() => {set_show_customer_popup(false)}}/>)}</div>
          )}
          {show_add_comment_popup && (
            <div className='w-full h-full'>{selected_invoice && (<AccountantCreateComment customer_name={customer_name} invoices={[selected_invoice]} sales_data={sales_data} onClose={()=>(set_show_add_comment_popup(false), fetchInvoiceData())} />)}</div>
          )}
        </div>
        ) : (
                ...
                <div className='flex justify-between space-x-2 w-full h-full'>
                    <div className='flex flex-col space-y-5 rounded-lg w-1/3 h-full bg-white p-5 italic'>
                        ...
                        <div className='flex flex-col space-y-1' >
                            ...
                            <div className='font-light text-gray-500 pl-2'>{selected_invoice.ref_no}</div>
                        </div>
                        <div className='flex flex-col space-y-1'>
...
export default AccountantInvoiceInfo;
